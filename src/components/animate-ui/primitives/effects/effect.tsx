...
function Effect({
  ...
  if (slide) {
    const offset = typeof slide === "boolean" ? DEFAULT_SLIDE_OFFSET : (slide.offset ?? DEFAULT_SLIDE_OFFSET);
    const direction =
      typeof slide === "boolean" ? DEFAULT_SLIDE_DIRECTION : (slide.direction ?? DEFAULT_SLIDE_DIRECTION);
  ...
  if (fade) {
    hiddenVariant.opacity =
      typeof fade === "boolean" ? DEFAULT_FADE_INITIAL_OPACITY : (fade.initialOpacity ?? DEFAULT_FADE_INITIAL_OPACITY);
    visibleVariant.opacity = typeof fade === "boolean" ? DEFAULT_FADE_OPACITY : (fade.opacity ?? DEFAULT_FADE_OPACITY);
  }

  if (zoom) {
    hiddenVariant.scale =
      typeof zoom === "boolean" ? DEFAULT_ZOOM_INITIAL_SCALE : (zoom.initialScale ?? DEFAULT_ZOOM_INITIAL_SCALE);
    visibleVariant.scale = typeof zoom === "boolean" ? DEFAULT_ZOOM_SCALE : (zoom.scale ?? DEFAULT_ZOOM_SCALE);
  }
  ...
  if (blur) {
    ...
    visibleVariant.filter =
      typeof blur === "boolean" ? `blur(${DEFAULT_BLUR_BLUR}px)` : `blur(${blur.blur ?? DEFAULT_BLUR_BLUR}px)`;
  }
  ...
  return (
    <>
      {array.map((child, index) => (
        <Effect delay={delay + index * holdDelay} key={child.key ?? index} {...props}>
          {child}
        </Effect>
      ))}
    </>
...
export { Effect, Effects, type EffectProps, type EffectsProps, type SlideDirection, type Slide, type Fade, type Zoom };
